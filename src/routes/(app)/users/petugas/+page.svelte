<script>
  ...
  let pages = [
    { name: 1, href: '/users/petugas?page=1'},
    { name: 2, href: '/users/petugas?page=2'},
    { name: 3, href: '/users/petugas?page=3'},
    { name: 4, href: '/users/petugas?page=4'},
    { name: 5, href: '/users/petugas?page=5'}
  ];
  ...
  const previous = () => {
    if (activeUrlPagination === "1") {
      goto(`/users/petugas?page=1`)
    }else if(activeUrlPagination === null) {
      goto(`/users/petugas?page=1`)
    }else {
      goto(`/users/petugas?page=${parseInt(activeUrlPagination)-1}`)
    }
  };
  const next = () => {
    let lastPageNumber = pages.slice(-1)[0].name;
    if (activeUrlPagination === String(lastPageNumber)) {
      goto(`/users/petugas?page=${lastPageNumber}`)
    }else if(activeUrlPagination === null) {
      goto(`/users/petugas?page=2`)
    }else {
      goto(`/users/petugas?page=${parseInt(activeUrlPagination)+1}`)
    }
  };
...
<div class="overflow-y-auto relative max-h-screen p-6 sm:p-10 space-y-6">
  <Breadcrumb class="mt-10" aria-label="Solid background breadcrumb example" solid>
    ...
    <BreadcrumbItem>Petugas</BreadcrumbItem>
  </Breadcrumb>
  <section class="px-4 py-6 bg-gray-50 dark:bg-gray-800 shadow rounded-lg">
    <div class="flex items-center justify-between">
      <p class="font-semibold text-2xl">Master Data Petugas RSI Aisyah</p>
      <Button size="sm" color="blue" on:click={() => goto("/users/petugas/tambah")}><Icon icon="material-symbols:add-circle" width="25" height="25" class="mr-2"/>Tambah Petugas</Button>
    </div>
    <hr class="my-5">
    ...
    <Table hoverable={true}>
      ...
      <TableBody>    
        {#each dummyPetugas as {nama, role}}
          <Petugas nama={nama} role={role}/>
        {/each}
      </TableBody>
    </Table>
