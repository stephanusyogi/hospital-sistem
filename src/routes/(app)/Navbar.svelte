...
<Navbar navClass="absolute z-40 px-2 sm:px-4 py-3 w-full border-b drop-shadow-md">
  ...
  <NavBrand href="/">
    <span class="self-center whitespace-nowrap text-md lg:text-lg xl:text-xl font-semibold dark:text-white">Selamat Datang, {user_data?.name}</span>
  </NavBrand>
  ...
  <Dropdown placement="bottom" triggeredBy="#avatar-menu">
    <DropdownHeader>
    <span class="block text-sm"> {user_data?.username} </span>
    <span class="block truncate text-sm font-medium"> {user_data?.role} </span>
    </DropdownHeader>
    <DropdownItem>
