...
<main>
  <form on:submit|preventDefault={handleSubmit}>
    <div class="flex items-center justify-between">
      <div>
        <p class="text-xl font-semibold">Form 12.16 Catatan Perkembangan Pasien Terintegrasi</p>
      ...
      <div class="my-2">
        <div class="flex items-center justify-between mb-2">
          ...
          <Button size="md" on:click={addInstruksi}>
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" class="mr-2" viewBox="0 0 24 24"><path fill="currentColor" d="M11 17h2v-4h4v-2h-4V7h-2v4H7v2h4v4Zm1 5q-2.075 0-3.9-.788t-3.175-2.137q-1.35-1.35-2.137-3.175T2 12q0-2.075.788-3.9t2.137-3.175q1.35-1.35 3.175-2.137T12 2q2.075 0 3.9.788t3.175 2.137q1.35 1.35 2.138 3.175T22 12q0 2.075-.788 3.9t-2.137 3.175q-1.35 1.35-3.175 2.138T12 22Z"/></svg> Tambah Instruksi
          </Button>
        </div>
        {#each instruksi as tag, i}
          <div class="flex items-center gap-4 py-2">
            <p>{numberInstruksi+i + ")"}</p>
            <Textarea rows="2" name="instruksi[]" placeholder="Masukan Diagnosa" bind:value={instruksi[i].instruksi}/>
            <Button on:click={() => deleteInstruksi(i)} size="xs" color="red"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="20" viewBox="0 0 24 24"><path fill="currentColor" d="M6 19c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V9c0-1.1-.9-2-2-2H8c-1.1 0-2 .9-2 2v10zM18 4h-2.5l-.71-.71c-.18-.18-.44-.29-.7-.29H9.91c-.26 0-.52.11-.7.29L8.5 4H6c-.55 0-1 .45-1 1s.45 1 1 1h12c.55 0 1-.45 1-1s-.45-1-1-1z"/></svg></Button>
          </div>
        {/each}
      </div>
    </div>
