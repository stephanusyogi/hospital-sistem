<script>
  export let createdAt, _id, subjective, objective, assessment, plan, sasaran, instruksi, ppa, identitas_ppa, verifikasi_dpjp, data;
  ...
  const handleDelete = (id) => {
    Swal.fire({
      title: 'Hapus Catatan Perkembangan?',
      ...
      if (result.isConfirmed) {
        ...
        await axios.delete(data.api_base+'/rekam-medis/catatan-perkembangan/'+id , { headers });
  ...
  const handleVerifikasi = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        await axios.put(data.api_base+'/rekam-medis/catatan-perkembangan/'+id, {verifikasi_dpjp: data.user_data.name} , { headers });
...
<tr>
  ...
  <td>
    <p><span class="font-semibold">S: </span>{subjective}</p>
    <p><span class="font-semibold">O: </span>{objective}</p>
    <p><span class="font-semibold">A: </span>{assessment}</p>
    <p><span class="font-semibold">P: </span>{plan}</p>
    <hr>
    <p><span class="font-semibold">Sasaran: </span>{sasaran}</p>
  </td>
  <td>{instruksi}</td>
  <td>{identitas_ppa} - {ppa}</td>
  <td>
    {#if verifikasi_dpjp === ''}
      {#if data?.user_data.role === 'Dokter'}
        <Button on:click={()=>handleVerifikasi(_id)}>Verifikasi</Button>
      {:else}
        Belum Diverifikasi
      {/if}
    {:else}
