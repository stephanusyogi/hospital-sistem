...
export const actions = {
  default: async ({request, cookies, params}) => {
    ...
    try {
      let dataLog = {
        'no_rekam_medis': no_rm,
        'keterangan': 'Menambahkan observasi cairan.',
        'nama': user_cookies.name,
        'role': user_cookies.role,
      }
      await axios.post(BACKEND_API+'/rekam-medis/log', dataLog ,config);

      await axios.post(BACKEND_API+'/rekam-medis/observasi-cairan', data, config)  
    ...
    throw redirect(303, `/rekam-medis/${no_rm}/observasi-cairan`)
  }
};