<script>
  export let _id, createdAt, nama_input, jumlah_input, nama_output, jumlah_output, data
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        await axios.delete(data.api_base+'/rekam-medis/observasi-cairan/'+id , { headers });
...
<tr>
  ...
  <td>{nama_input}</td>
  <td>{jumlah_input} CC</td>
  <td>{nama_output}</td>
  <td>{jumlah_output} CC</td>
  <td>
