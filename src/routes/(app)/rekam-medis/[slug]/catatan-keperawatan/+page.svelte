<script>
  ...
  let pages = [
    { name: 1, href: `/rekam-medis/${no_rm}/catatan-keperawatan?page=1`},
    { name: 2, href: `/rekam-medis/${no_rm}/catatan-keperawatan?page=2`},
    { name: 3, href: `/rekam-medis/${no_rm}/catatan-keperawatan?page=3`},
    { name: 4, href: `/rekam-medis/${no_rm}/catatan-keperawatan?page=4`},
    { name: 5, href: `/rekam-medis/${no_rm}/catatan-keperawatan?page=5`},
  ];
  $:{ 
      pages.forEach((page)=>{
  ...
  const previous = () => {
    if (activeUrlPagination === "1") {
      goto(`/rekam-medis/${no_rm}/catatan-keperawatan?page=1`)
    }else if(activeUrlPagination === null) {
      goto(`/rekam-medis/${no_rm}/catatan-keperawatan?page=1`)
    }else {
      goto(`/rekam-medis/${no_rm}/catatan-keperawatan?page=${parseInt(activeUrlPagination)-1}`)
    }
  };
  const next = () => {
    let lastPageNumber = pages.slice(-1)[0].name;
    if (activeUrlPagination === String(lastPageNumber)) {
      goto(`/rekam-medis/${no_rm}/catatan-keperawatan?page=${lastPageNumber}`)
    }else if(activeUrlPagination === null) {
      goto(`/rekam-medis/${no_rm}/catatan-keperawatan?page=2`)
    }else {
      goto(`/rekam-medis/${no_rm}/catatan-keperawatan?page=${parseInt(activeUrlPagination)+1}`)
    }
  };
...
<div>
  <div class="flex flex-wrap items-centere justify-between">
    <div>
      <p class="font-medium text-xl">Catatan Keperawatan</p>
    </div>
    <Button href="catatan-keperawatan/tambah" color="green">
...
<Table hoverable={true} shadow>
  <TableHead>
    ...
    <TableHeadCell class="text-center">Identitas Perawat</TableHeadCell>
  </TableHead>
  <TableBody> 
    {#each dummyCatatan as { tgl_jam, catatan, identitas }}
