<script>

  import { Button} from "flowbite-svelte";

</script>

<main>
  <div class="flex items-center justify-between">
    <div>
      <p class="text-xl font-semibold">Asesmen Medis Awal</p>
    </div>
    <Button href="asesmen-medis-awal/update" color="green">
      <svg xmlns="http://www.w3.org/2000/svg" class="mr-2" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="m19.3 8.925l-4.25-4.2l1.4-1.4q.575-.575 1.413-.575t1.412.575l1.4 1.4q.575.575.6 1.388t-.55 1.387L19.3 8.925ZM17.85 10.4L7.25 21H3v-4.25l10.6-10.6l4.25 4.25Z"/></svg>
      Update</Button>
  </div>
  <hr class="my-5">
  <div class="overflow-y-auto max-h-screen">
    <div class="border border-slate-300 rounded p-4 w-full">
      <div class="mb-2">
        <p class="text-lg font-semibold italic">Anamnesa</p>
        <div class="grid grid-cols-2 gap-6 py-2">
          <div class="grid grid-cols-2 gap-6 border p-4 text-sm col-span-2">
            <p class="font-medium">Keluhan Utama:</p>
            <p class="font-light text-right">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vel ex neque. Proin mauris eros, hendrerit euismod faucibus sed, molestie vitae massa. Etiam consectetur eleifend fermentum.</p>
          </div>
          <div class="grid grid-cols-2 gap-6 border p-4 text-sm col-span-2">
            <p class="font-medium">Riwayat Penyakit Sekarang:</p>
            <p class="font-light text-right">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vel ex neque. Proin mauris eros, hendrerit euismod faucibus sed, molestie vitae massa. Etiam consectetur eleifend fermentum.</p>
          </div>
          <div class="grid grid-cols-2 gap-6 border p-4 text-sm col-span-2">
            <p class="font-medium">Riwayat Pengobatan:</p>
            <p class="font-light text-right">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vel ex neque. Proin mauris eros, hendrerit euismod faucibus sed, molestie vitae massa. Etiam consectetur eleifend fermentum.</p>
          </div>
          <div class="grid grid-cols-2 gap-6 border p-4 text-sm col-span-2">
            <p class="font-medium">Riwayat Alergi:</p>
            <p class="font-light text-right">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vel ex neque. Proin mauris eros, hendrerit euismod faucibus sed, molestie vitae massa. Etiam consectetur eleifend fermentum.</p>
          </div>
        </div>
      </div>
      <div class="mb-2">
        <p class="text-lg font-semibold italic">Pemeriksaan Fisik</p>
        <div class="grid grid-cols-2 gap-6 py-2">
          <div class="grid grid-cols-2 gap-6 border p-4 text-sm col-span-2">
            <p class="font-medium">Keadaan Umum:</p>
            <p class="font-light text-right">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vel ex neque. Proin mauris eros, hendrerit euismod faucibus sed, molestie vitae massa. Etiam consectetur eleifend fermentum.</p>
          </div>
          <div class="grid grid-cols-2 gap-6 border p-4 text-sm col-span-2">
            <p class="font-medium">Kepala/Leher:</p>
            <p class="font-light text-right">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vel ex neque. Proin mauris eros, hendrerit euismod faucibus sed, molestie vitae massa. Etiam consectetur eleifend fermentum.</p>
          </div>
          <div class="grid grid-cols-2 gap-6 border p-4 text-sm col-span-2">
            <p class="font-medium">Ekstremitas:</p>
            <p class="font-light text-right">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vel ex neque. Proin mauris eros, hendrerit euismod faucibus sed, molestie vitae massa. Etiam consectetur eleifend fermentum.</p>
          </div>
          <div class="grid grid-cols-2 gap-6 border p-4 text-sm col-span-2">
            <p class="font-medium">Genitalia:</p>
            <p class="font-light text-right">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vel ex neque. Proin mauris eros, hendrerit euismod faucibus sed, molestie vitae massa. Etiam consectetur eleifend fermentum.</p>
          </div>
          <div class="grid grid-cols-2 gap-6 border p-4 text-sm col-span-2">
            <p class="font-medium col-span-2">Kesadaran & GCS:</p>
            <div class="grid grid-cols-2 gap-4 col-span-2">
              <div class="grid grid-cols-2 gap-2">
                <p class="font-medium">Kesadaran:</p>
                <p class="font-light text-right">Lorem ipsum dolor sit amet, consectetur adipiscing elit.</p>
              </div>
              <div class="grid grid-cols-2 gap-2">
                <p class="font-medium">GCS:</p>
                <p class="font-light text-right">Lorem ipsum dolor sit amet, consectetur adipiscing elit.</p>
              </div>
            </div>
          </div>
          <div class="grid grid-cols-2 gap-6 border p-4 text-sm col-span-2">
            <p class="font-medium col-span-2">Status Gizi & BB:</p>
            <div class="grid grid-cols-2 gap-4 col-span-2">
              <div class="grid grid-cols-2 gap-2">
                <p class="font-medium">Status Gizi:</p>
                <p class="font-light text-right">Lorem ipsum dolor sit amet, consectetur adipiscing elit.</p>
              </div>
              <div class="grid grid-cols-2 gap-2">
                <p class="font-medium">BB:</p>
                <p class="font-light text-right">Lorem ipsum dolor sit amet, consectetur adipiscing elit.</p>
              </div>
            </div>
          </div>
          <div class="grid grid-cols-2 gap-6 border p-4 text-sm col-span-2">
            <p class="font-medium col-span-2">Thorax:</p>
            <div class="grid grid-cols-2 gap-4 col-span-2">
              <div class="grid grid-cols-2 gap-2">
                <p class="font-medium">COR:</p>
                <p class="font-light text-right">Lorem ipsum dolor sit amet, consectetur adipiscing elit.</p>
              </div>
              <div class="grid grid-cols-2 gap-2">
                <p class="font-medium">Pulmo:</p>
                <p class="font-light text-right">Lorem ipsum dolor sit amet, consectetur adipiscing elit.</p>
              </div>
            </div>
          </div>
          <div class="grid grid-cols-2 gap-6 border p-4 text-sm col-span-2">
            <p class="font-medium col-span-2">Abdomen:</p>
            <div class="grid grid-cols-2 gap-4 col-span-2">
              <div class="grid grid-cols-2 gap-2">
                <p class="font-medium">Inspeksi:</p>
                <p class="font-light text-right">Lorem ipsum dolor sit amet, consectetur adipiscing elit.</p>
              </div>
              <div class="grid grid-cols-2 gap-2">
                <p class="font-medium">Palpasi:</p>
                <p class="font-light text-right">Lorem ipsum dolor sit amet, consectetur adipiscing elit.</p>
              </div>
            </div>
            <div class="grid grid-cols-2 gap-4 col-span-2">
              <div class="grid grid-cols-2 gap-2">
                <p class="font-medium">Auskultasi:</p>
                <p class="font-light text-right">Lorem ipsum dolor sit amet, consectetur adipiscing elit.</p>
              </div>
              <div class="grid grid-cols-2 gap-2">
                <p class="font-medium">Perkusi:</p>
                <p class="font-light text-right">Lorem ipsum dolor sit amet, consectetur adipiscing elit.</p>
              </div>
            </div>
          </div>
          <div class="grid grid-cols-2 gap-6 border p-4 text-sm col-span-2">
            <p class="font-medium col-span-2">Vital Sign:</p>
            <div class="grid grid-cols-2 gap-4 col-span-2">
              <div class="grid grid-cols-2 gap-2">
                <p class="font-medium">Tensi:</p>
                <p class="font-light text-right">Lorem ipsum dolor sit amet, consectetur adipiscing elit.</p>
              </div>
              <div class="grid grid-cols-2 gap-2">
                <p class="font-medium">Nadi:</p>
                <p class="font-light text-right">Lorem ipsum dolor sit amet, consectetur adipiscing elit.</p>
              </div>
            </div>
            <div class="grid grid-cols-2 gap-4 col-span-2">
              <div class="grid grid-cols-2 gap-2">
                <p class="font-medium">Suhu:</p>
                <p class="font-light text-right">Lorem ipsum dolor sit amet, consectetur adipiscing elit.</p>
              </div>
              <div class="grid grid-cols-2 gap-2">
                <p class="font-medium">RR:</p>
                <p class="font-light text-right">Lorem ipsum dolor sit amet, consectetur adipiscing elit.</p>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div class="mb-2">
        <p class="text-lg font-semibold italic">Pemeriksaan Penunjang</p>
        <div class="grid grid-cols-2 gap-6 py-2">
          <div class="grid grid-cols-2 gap-6 border p-4 text-sm col-span-2">
            <p class="font-medium">Laboratorium:</p>
            <p class="font-light text-right">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vel ex neque. Proin mauris eros, hendrerit euismod faucibus sed, molestie vitae massa. Etiam consectetur eleifend fermentum.</p>
          </div>
          <div class="grid grid-cols-2 gap-6 border p-4 text-sm col-span-2">
            <p class="font-medium">Radiologi:</p>
            <p class="font-light text-right">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vel ex neque. Proin mauris eros, hendrerit euismod faucibus sed, molestie vitae massa. Etiam consectetur eleifend fermentum.</p>
          </div>
          <div class="grid grid-cols-2 gap-6 border p-4 text-sm col-span-2">
            <p class="font-medium">Lain-Lain:</p>
            <p class="font-light text-right">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vel ex neque. Proin mauris eros, hendrerit euismod faucibus sed, molestie vitae massa. Etiam consectetur eleifend fermentum.</p>
          </div>
        </div>
      </div>
      <div class="mb-2">
        <p class="text-lg font-semibold italic">Masalah</p>
        <div class="grid grid-cols-2 gap-6 py-2">
          <div class="grid grid-cols-2 gap-6 border p-4 text-sm col-span-2">
            <p class="font-medium">Uraian Masalah:</p>
            <p class="font-light text-right">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vel ex neque. Proin mauris eros, hendrerit euismod faucibus sed, molestie vitae massa. Etiam consectetur eleifend fermentum.</p>
          </div>
        </div>
      </div>
      <div class="mb-2">
        <p class="text-lg font-semibold italic">Diagnosis</p>
        <div class="grid grid-cols-2 gap-6 py-2">
          <div class="grid grid-cols-2 gap-6 border p-4 text-sm col-span-2">
            <p class="font-medium">Uraian Diagnosis:</p>
            <p class="font-light text-right">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vel ex neque. Proin mauris eros, hendrerit euismod faucibus sed, molestie vitae massa. Etiam consectetur eleifend fermentum.</p>
          </div>
        </div>
      </div>
      <div class="mb-2">
        <p class="text-lg font-semibold italic">Rencana Asuhan Pemeriksaan Penunjang</p>
        <div class="grid grid-cols-2 gap-6 py-2">
          <div class="grid grid-cols-2 gap-6 border p-4 text-sm col-span-2">
            <p class="font-medium">Uraian Rencana Asuhan:</p>
            <p class="font-light text-right">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vel ex neque. Proin mauris eros, hendrerit euismod faucibus sed, molestie vitae massa. Etiam consectetur eleifend fermentum.</p>
          </div>
          <div class="grid grid-cols-2 gap-6 border p-4 text-sm col-span-2">
            <p class="font-medium">Terapi:</p>
            <p class="font-light text-right">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vel ex neque. Proin mauris eros, hendrerit euismod faucibus sed, molestie vitae massa. Etiam consectetur eleifend fermentum.</p>
          </div>
          <div class="grid grid-cols-2 gap-6 border p-4 text-sm col-span-2">
            <p class="font-medium">Tindakan/prosedur khusus/operasi:</p>
            <p class="font-light text-right">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vel ex neque. Proin mauris eros, hendrerit euismod faucibus sed, molestie vitae massa. Etiam consectetur eleifend fermentum.</p>
          </div>
          <div class="grid grid-cols-2 gap-6 border p-4 text-sm col-span-2">
            <p class="font-medium">Nutrisi:</p>
            <p class="font-light text-right">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vel ex neque. Proin mauris eros, hendrerit euismod faucibus sed, molestie vitae massa. Etiam consectetur eleifend fermentum.</p>
          </div>
          <div class="grid grid-cols-2 gap-6 border p-4 text-sm col-span-2">
            <p class="font-medium">Konsultasi/rehabilitasi:</p>
            <p class="font-light text-right">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vel ex neque. Proin mauris eros, hendrerit euismod faucibus sed, molestie vitae massa. Etiam consectetur eleifend fermentum.</p>
          </div>
        </div>
      </div>
      <div class="mb-2">
        <p class="text-lg font-semibold italic">Sasaran</p>
        <div class="grid grid-cols-2 gap-6 py-2">
          <div class="grid grid-cols-2 gap-6 border p-4 text-sm col-span-2">
            <p class="font-medium">Uraian Sasaran:</p>
            <p class="font-light text-right">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vel ex neque. Proin mauris eros, hendrerit euismod faucibus sed, molestie vitae massa. Etiam consectetur eleifend fermentum.</p>
          </div>
        </div>
      </div>
      <div class="mb-2">
        <p class="text-lg font-semibold italic">Rencana Monitoring / Follow Up</p>
        <div class="grid grid-cols-2 gap-6 py-2">
          <div class="grid grid-cols-2 gap-6 border p-4 text-sm col-span-2">
            <p class="font-medium">Uraian:</p>
            <p class="font-light text-right">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vel ex neque. Proin mauris eros, hendrerit euismod faucibus sed, molestie vitae massa. Etiam consectetur eleifend fermentum.</p>
          </div>
        </div>
      </div>
      <div class="mb-2">
        <p class="text-lg font-semibold italic">Efek Samping / Komplikasi yang Mungkin Terjadi / Hasil yang Tidak Diharapkan</p>
        <div class="grid grid-cols-2 gap-6 py-2">
          <div class="grid grid-cols-2 gap-6 border p-4 text-sm col-span-2">
            <p class="font-medium">Uraian:</p>
            <p class="font-light text-right">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vel ex neque. Proin mauris eros, hendrerit euismod faucibus sed, molestie vitae massa. Etiam consectetur eleifend fermentum.</p>
          </div>
        </div>
      </div>
      <div class="mb-2">
        <p class="text-lg font-semibold italic">Hasil yang Diharapkan / Outcome (Prognosa)</p>
        <div class="grid grid-cols-2 gap-6 py-2">
          <div class="grid grid-cols-2 gap-6 border p-4 text-sm col-span-2">
            <p class="font-medium">Uraian:</p>
            <p class="font-light text-right">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vel ex neque. Proin mauris eros, hendrerit euismod faucibus sed, molestie vitae massa. Etiam consectetur eleifend fermentum.</p>
          </div>
        </div>
      </div>
      <div class="mb-2">
        <p class="text-lg font-semibold italic">Pengkajian Rencana Pemulangan Pasien</p>
        <p class="text-md font-medium italic">Catatan: jika satu terpenuhi maka pasien membutuhkan perencanaan pemulangan khusus.</p>
        <div class="grid grid-cols-2 gap-6 py-2">
          <div class="grid grid-cols-2 gap-6 border p-4 text-sm">
            <p class="font-medium">Usia lanjut(60 tahun atau lebih):</p>
            <p class="font-light text-right">Tidak</p>
          </div>
          <div class="grid grid-cols-2 gap-6 border p-4 text-sm">
            <p class="font-medium">Hambatan mobilitasi:</p>
            <p class="font-light text-right">Tidak</p>
          </div>
          <div class="grid grid-cols-2 gap-6 border p-4 text-sm">
            <p class="font-medium">Membutuhkan perawatan medis dan perawatan berkelanjutan:</p>
            <p class="font-light text-right">Tidak</p>
          </div>
          <div class="grid grid-cols-2 gap-6 border p-4 text-sm">
            <p class="font-medium">Tergantung dengan orang lain dalam aktivitas harian:</p>
            <p class="font-light text-right">Tidak</p>
          </div>
        </div>
      </div>
    </div>
  </div>
</main>