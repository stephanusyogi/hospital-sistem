<script>
  import { Button, Checkbox, Input, Label, Modal, Radio, Search, Select, Textarea } from "flowbite-svelte";
  export let no_rm, nama_pasien, jenis_kelamin, ttl, alamat_ktp;
...
<button class={userCardClass} on:click={() => detailModal = true}>
  <p class="font-bold text-right text-gray-700 dark:text-gray-400 leading-tight">No. RM: {no_rm}</p>
  <hr class="my-2 border-gray-800">
  <p class="font-normal text-gray-700 dark:text-gray-400 leading-tight">
    <span class="font-semibold">Nama Lengkap</span>: {nama_pasien} <span class="italic">, {jenis_kelamin}.</span>
  </p>
  <p class="font-normal text-gray-700 dark:text-gray-400 leading-tight">
    <span class="font-semibold">TTL</span>: {ttl}
  </p>
  <p class="font-normal text-gray-700 dark:text-gray-400 leading-tight">
    <span class="font-semibold">Alamat</span>: {alamat_ktp}
  </p>
</button>

<Modal title="Detail Singkat Pasien" bind:open={detailModal} size="xl">
  <form on:submit|preventDefault={handleUpdateDataPasien}>
    <div class="grid grid-cols-1 md:grid-cols-4 gap-2">
      <div class="px-4">
        <div class="group mb-4">
          <Label for='no_rekam_medis' class='block mb-2'>Nomor Rekam Medis:</Label>
          <Input id="no_rekam_medis" name="no_rekam_medis" placeholder="Masukkan nomor rekam medis pasien" value={no_rm}/>
        </div>
        <div class="group mb-4">
          <Label for='nama_lengkap' class='block mb-2'>Nama Lengkap Pasien:</Label>
          <Input id="nama_lengkap" name="nama_lengkap" placeholder="Masukkan nama lengkap pasien" value={nama_pasien}/>
        </div>
        <div class="group mb-4">
          <Label for='umur' class='block mb-2'>Umur:</Label>
          <Input id="umur" name="umur" placeholder="Masukkan umur pasien" value="Lorem Ipsum"/>
        </div>
        <div class="group mb-4">
      ...
      <div class="px-4">
        ...
        <div class="group mb-4">
          ...
          <Textarea name="alamat_tempat_tinggal" id="alamat_tempat_tinggal" placeholder="Masukkan alamat tempat tinggal pasien" rows="1" value={alamat_ktp}/>
        </div>
        <div class="group mb-4">
          <Label for='alamat_ktp' class='block mb-2'>Alamat KTP:</Label>
          <Textarea name="alamat_ktp" id="alamat_ktp" placeholder="Masukkan alamat sesuai ktp pasien" rows="1" value="Lorem ipsum"/>
        </div>
      </div>
      <div class="px-4">
          <div class="group mb-4">
      ...
      <div class="px-4">
        <div class="group mb-4">
          <p class="font-semibold text-gray-500 dark:text-gray-400">Asuransi Pasien:</p>
          <p class="text-base leading-relaxed text-gray-500 dark:text-gray-400">Umum/Mandiri</p>
        </div>
          <div class="w-64">
            <Label for="tipePasien">Ubah Tipe Asuransi Pasien:</Label>
          ...
          {:else}
            <p class="font-semibold text-md">
              Silahkan pilih tipe asuransi pasien terlebih dahulu.
            </p>
          {/if}
      </div>
    </div>
    ...
    <div class="flex flex-wrap justify-center items-center gap-4">
      <Button type="submit" color="blue">Simpan Perubahan Data</Button>
      <Button color="green" on:click={() => goto(`/pendaftaran/${no_rm}`)}>Lanjutkan ke Pendaftaran Rawat Inap</Button>
    </div>
  </form>
</Modal>